...
//var bing = new client.BingSpeechClient('cac0008c38034736b48aa300ff504d4e');
...
function queryCollection(session,query) {
    session.send(`Querying collection through index:\n${config.collection.id}`);
	var querydata = 'SELECT * FROM c.' + query;
    session.send(querydata);
    return new Promise((resolve, reject) => {
        client.queryDocuments(
            ...
            if (err) reject(err)
                ...
                for (var queryResult of results) {
                    ...
                    session.send(`\tQuery returned ${resultString}`);
					  var request = new sql.Request();
					request.query("Insert into Student (Name,Age,Result) values ('"+query+"',13000,'"+resultString+"')")
					.then(function () {
					...
					fs.appendFile("a.txt", resultString + "\r\n", function(err) {
    if(err) {
        return console.log(err);
    }
...
bot.dialog('/menu', [
    function (session) {
        builder.Prompts.choice(session, "What demo would you like to run?", "Jagan|Deepak|(quit)");
    },
    function (session, results) {
	fs.appendFile("a.txt", results.response.entity +"\r\n", function(err) {
    if(err) {
        return console.log(err);
    }
    ...
    console.log("The file was saved!");
		...
		getDatabase()
		 .then(() => queryCollection(session,results.response.entity))
            // Launch demo dialog
